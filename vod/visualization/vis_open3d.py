...
Constructor which prepared the 3D plot in the requested frame.
        :param frame_data:
        """
        self.vis = None #类里共用的open3d viser
...
This method plots the lidar pcl on the requested frame.
        ...
        add_pointcloud(lidar_points_camera_frame,self.vis)
...
This method plots the radar pcl on the requested frame.
        ...
        add_sphere(points = np.asarray(radar_points_camera_frame[:, :3], dtype=float),
                            color = sns.color_palette(["red"])[0],
                            vis = self.vis)
...
This method plots the radar radial velocity vectors for each radar point in the requested frame.
        ...
        add_radar_radial_velocity(pc_radar, velocity_vectors, sns.color_palette([color])[0], self.vis)

    def plot_annotations(self, class_colors=label_color_palette_open3d, class_width=label_line_width_3d):
        """
This method plots the annotations in the requested frame.
        ...
        for box in bboxes:
            ...
            add_one_corner_box(corners_object, sns.color_palette([object_class_color])[0],self.vis)
...
This method displays the plot with the specified arguments.
        ...
        window_x = 4000
        window_y = 0 + 400
        window_width = int(1920*1.3 )
        window_height = int(1080 *1.3)

        self.vis.create_window(width=window_width, height=window_height, left=window_x, top=window_y)
